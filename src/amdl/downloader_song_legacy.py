...
class DownloaderSongLegacy(DownloaderSong):
    """Legacy codec downloader: uses webplayback-only stream selection
    and ffmpeg's built-in `-decryption_key` for single-step decrypt+remux.

    This preserves the v2.4.2 behavior that correctly handled Apple's
    fragmented MP4 streams (flavor 28:ctrp256 / 32:ctrp64) without
    losing the first ~15 seconds of audio.
    """
    ...
    def get_stream_info(self, webplayback: dict) -> StreamInfo:
        # AAC_LEGACY always uses flavor 28:ctrp256
        flavor = "28:ctrp256"
